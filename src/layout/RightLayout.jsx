...
export default function RightLayout(props){
    ...
    useEffect(() => {
        const callData = async () => {
            if (token && playlist) {
                const response = await fetchPlaylistItems(token, list_song ? list_song : playlist?.items[0]?.id);
                setList(response);
            }
        }
        callData();
    ...
    return (
        <div className="flex flex-col w-full pt-3 h-[85vh] bg-gradient-to-b from-[#39E08C] to-[black] to-90% overflow-y-auto text-white ml-2 mt-3 rounded-t-xl" onScroll={handleScroll}>
            <div ref={playlistEl} className="text-7xl mb-20 font-bold">
                {playlist?.name ? (
                    <div className="flex flex-row items-end transition ease-in-out duration-300 py-5 gap-3 pl-10">
                        <img src={playlist?.img} alt="song" className="w-52 h-w-52 shadow-2xl"></img>
                        <div className="flex flex-col gap-3">
                            <div>
                                <p className="text-sm">Playlist</p>
                                <p className="text-6xl">{playlist?.name}</p>
                            </div>
                                <p className="text-base">{playlist?.owner} ▪ {playlist?.tracks} lagu, <span className="opacity-70">1 jam 45 menit</span></p>
                        </div>
                    </div>
                ) : (
                <div className="flex flex-row items-end transition ease-in-out duration-300 py-5 gap-3 pl-10">
                    <img src={playlist?.items[0]?.images[0].url} alt="song" className="w-52 h-w-52 shadow-2xl"></img>
                    <div className="flex flex-col gap-3">
                    <div>
                        <p className="text-sm">Playlist</p>
                        <p className="text-6xl">{playlist?.items[0].name}</p>
                    </div>
                        <p className="text-base">{playlist?.items[0].owner.display_name} ▪ {playlist?.items[0].tracks.total} lagu, <span className="opacity-70">1 jam 45 menit</span></p>
                    </div>
                </div>
                )}
            </div>
            <div className="flex flex-col relative before:absolute before:bg-black before:opacity-50 before:top-0 before:left-0 before:right-0 before:bottom-0">
                ...
                <AnimatePresence>
                        {
                            state.playlist && playlist.name ? (
                                    <motion.div initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity:0}} transition={{ ease: "easeInOut", duration: .3 }} className="flex flex-row items-end fixed top-3 bg-[#228554] w-[83%] rounded-tl-xl z-50 py-3 gap-3 pl-10">
                        ...
                        :   state.playlist && (
                            <motion.div initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity:0}} transition={{ ease: "easeInOut", duration: .3 }} className="flex flex-row items-end fixed top-3 bg-[#228554] w-[83%] rounded-tl-xl z-50 py-3 gap-3 pl-10">
                                <img src={playlist?.items[0]?.images[0]?.url} alt="song" className="w-10 h-10 shadow-2xl"></img>
                                <div className="flex flex-col pl-3">
                                    <p className="text-xl">{playlist?.items[0]?.name}</p>
                                    <p className="text-sm ">{playlist?.items[0]?.owner?.display_name}</p>
                                </div>
                            </motion.div>
                            ) 
                        }
                ...
                <AnimatePresence>
                        {
                            state.description && (
                                    <motion.div initial={{opacity: 0}} animate={{opacity: .9}} exit={{opacity:0}} transition={{ ease: "easeInOut", duration: .1 }} className="flex fixed z-10 bg-[#165D3A] w-[83%] top-[83px] mb-3 items-center">
                                        <div className="p-3 ml-6 ">#</div>
                                        <div className="ml-9">Judul</div>
                                        <IoTimeSharp className="ml-[70%]"/>
                                    </motion.div>
                ...
                list?.data?.items.map((v, i) => {
                    ...
                    const class_name = `flex ${i < 9 ? 'ml-[18px]' : 'ml-[10px]'} p-3 gap-3 w-full hover:bg-[#2B3731]`;
                        if (v.track.preview_url){
                            return (
                                <div key={v?.track?.id} className="flex relative items-center ml-5" onClick={()=> {
                                    ...
                                    <div className={class_name}>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs opacity-50">{v.track.artists.map(v => v.name).join(", ")}</p>
                                        </div>
                                    </div>
                                </div>
